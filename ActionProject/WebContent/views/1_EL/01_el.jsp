...
<body>
	...
	<h3>2. El을 아용하여 보다 쉽게 Scope에 저장된 값들 출력하기</h3>
	<p>
		EL은 getXXX를 통해 값을 빼올 필요 없이 key값만 제시하면 바로 접근 가능<br>
		내부적으로 해당 scope영역에 해당하는 해당 key값의 value값을 가져올 수 있다. <br>
		기본적으로 EL은 JSP내장객체를 구분하지 않고 자동으로 모든 내장객체에 키값을 검색해서 존재하는 경우 해당값을 가져옴
	</p>
	<p>
		학원명 : ${academy } <br>
		강의장 : ${classRoom} <br>
		강사정보 : ${teacher.name }, ${teacher.gender }, ${teacher.job }
		<!-- 
			student라는 key값에 접근했을 때 value값이 Person객체임
			해당 Person객체의 각 필드에 담긴 값들을 출력하고자 한다면 key값.필드명으로 접근하면 됨
			* 내부적으로 해당 필드의 getter메소드를 찾아서 실행하는 구조(getter메소드가 없다면 실행이 안됨)
			  -> 기존접근 방식 : student.getName();
		 -->
		 <ul>
	...
	<h3>3. EL 사용할 때 내장객체들에 저장된 key값이 동일한 CASE</h3>
	scope값 : ${ scope } <br>
	<!-- 
		EL은 공유범위가 제일 작은 Scope에서부터 키값을 검색해 나감
		page -> request -> session -> application순으로 속성을 찾아나간다.
		현재 찾고있는 scope범위 안에서 내가 찾고자하는 속성(key값)이 존재한다면, 반복을 멈추고 해당 속성의 속성값을 출력함
