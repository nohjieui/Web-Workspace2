...
* 주의사항
 - 변수의 타입을 별도로 지정하지 않음
 - 반드시 해당 변수의 담아두고자하는 초기값(value)속성을 무조건 추가시켜줘야함(선언과 동시에 초기화 해줘야함)
</pre>
	...
	num1 변수값 : ${num2 } 또는 ${requestScope.num2 } <br>
	
	<c:set var="result" scope="session" value="${num1 + num2 }"/>
	result 변수값 : ${result } <br><br>
	
	<!-- 
		변수명만 제시하면 공유범위가 가장 작은곳에서부터 큰곳으로 순차적으로 찾아지게됨.
		티가나지는 않지만 속도가 조금 느려질 수가 있다.
		따라서 스코프영역.변수명을 작성하는 것을 권장한다.
	 -->
...
* 변수 삭제(&lt; c:remove var="제거하고자하는 변수명" scope="스코프영역지정(생략가능)" &gt;)
 ...
 - scope지정 생략시 모든 sope에서 해당 변수를 다~ 찾아서 제거함.
 => 즉, 해당 scope에 .removeAttribute라는 메서드를 이용해서 제거하는것이라고 생각하면 됨	 
	...
	request에 있는 result변수 삭제 후 result : ${result } <br><br>
...
* 변수(데이터) 출력 : (&lt;c:out value="출력하고자 하는 값" default="기본값(생략가능)" escapeXml="true/false(기본값:true, 생략가능)" &gt;)
 ...
 - escapeXml : 태그로써 해석해서 출력할지 여부(생략가능, 기본값:true)
	...
	<c:out value="${outTest }" /> <br> <!-- escapeXml 생략시 기본값 true == 태그 해석이 안됨(문자열로 취급함) -->
...
<h3>3. 조건문 - choose(&lt;c:choose&gt; , &lt;c:when text="조건" &gt; , &lt;c:otherwise&gt;)</h3>

<pre>
 - JAVA의 if-else, if-else if 또는 switch문과 비슷한 역할을 하는 태그
 - 각 조건들은 c:choose태그의 하위요소로 c:when, c:otherwise를 통해서 작성
</pre>
	...
	<c:choose>
		...
		<c:otherwise>
			<b>num1은 10 혹은 num2는 2와 일치하지 않습니다.</b> <br>
		</c:otherwise>
	</c:choose>
...
향상된 반목문 - (&lt;c:forEach var="변수명" item="순차적으로 접근할 배열, 컬렉션" varStatus="현재접근된요소의 상태값을 저장하는 변수명(생략가능)"&gt;)
...
=> varStatus : 현재 접근된 요소의 상태값을 보관할 변수명(index, count, first, last)(생략 가능)
	...
	<%
		...
		list.add(new Person("노지의","여","학생"));
		list.add(new Person("노지의1","여","백수"));
		list.add(new Person("노지의2","여","디자이너"));
...
<pre>
&lt;c:forTokens var="각값을 보관할 변수" item="분리시키고자하는 문자열" delims="구분자"&gt;
...
&lt;/c:url&gt;
 ...
 - 특정조건에 따라 파라미터값을 다르게 보내야할 때 자주 사용됨
</pre>
