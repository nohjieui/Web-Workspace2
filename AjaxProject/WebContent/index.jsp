...
<body>
	...
	<p>
		...
		=> 응답페이지가 돌아와야 볼 수 있으며 응답페이지가 호출되기 전까지 아무요청 하지못함<br>
		비동기식 요청을 보내기 위해서는 Ajax라는 기술이 필요함
		<br><br>
		
		* 동기식/비동기식 <br>
		- 동기식 : 요청 처리 후 그에 해당하는 응답페이지가 돌아와야만 그 다음 작업 가능함 <br>
		만약 서버에서 호출된 결과까지의 시간이 지연되면계속 기다려야함(흰화면으로 보이게됨) <br>
		전체 페이지가 리로드됨(새로고침 즉, 페이지가 기본적으로 한번 깜빡거림) <br><br>
		
		- 비동기식 : 현제 페이지를 그대로 유지하면서 중간중간 추가적인 요청을 보내줄 수 있음 <br>
		요청을 한다고해서 다른 페이지로 넘어가지 않음(현재 페이지 유지) <br>
		요청을 보내놓고 그에 해당하는 응답이 돌아올 때 까지 현재 페이지에서 다른 작업을 할 수 있음 <br>
		ex) 아이디 중복체크기능, 검색어 자동완성기능
		<br><br>
		
		* 비동기식의 단점 <br>
		- 현재 페이지에 지속적으로 리소스가 쌓임 => 페이지가 느려질 수 있음 <br>
		- 페이지 내 복잡도가 크게 증가함 -> 에러발생시 대처가 어려워질 수 있음 <br>
	...
	<script>
		function ajaxTest1(){
			...
			// 2. XMLResquest 객체를 설정하기
			// open()함수를 이용해서 통신값을 설정
			// 첫번째 매개변수 : get/post , 두번째 매개변수 : 요청을 보내는 주소, [세번째 매개변수 : 동기식, 비동기식 설정]
			...
			xhr.onreadystatechange = function(){
				...
				if(xhr.readyState == 4){ // 응답완료
					...
					else if(xhr.statuse == 404){ // 찾는 페이지(url)이 존재하지 않는다.
	...
	<pre>
		...
		* 주요속성
		- url : 요청할 url(필수)
		- type | method : 요청할 전송방식(get/post, 생략시 기본값은 get)
		- data : 요청시 전달할 값
		- success  : ajax통신 성공시 실행할 함수를 정의
		- error    : ajax통식 실패시 실행할 함수를 정의
		- complete : ajax통식 실패했든, 성공했든 무조건 실행할 함수를 정의
		...
		* 부수적인 속성
		...
		- global : 기본 이벤트 사용 여부 => 버퍼링 값이 시작과 끝을 나타낼 때, 선처리 작업시 사용함
		- password : 서버에 접속권한이 필요한 경우 설정
		- processDta : 서버로 보내는 값에 대한 형태 설정 여부(기본데이터 원하는 경우 false)
		- timeout : 서버 요청시 응답 대기시간 설정(ms 단위)
	</pre>
